...
test("DashboardView layout v2: Hero+Trend top row, Heatmap+Context share a row, deduped context", () => {
  // Layout v2 pairs Hero and Trend on the top row (3:3); the Activity Heatmap
  // and Context Breakdown then share a row (Heatmap cols 1-2, Context cols 3-6);
  // and the provider drill-down's inline context is suppressed in favor of the
  // standalone ContextCard.
  const src = readFile(viewPath);
  ...
  assert.ok(
    src.includes('activityHeatmapBlock ? "lg:col-span-4 min-w-0" : "lg:col-span-6 min-w-0"'),
    "expected Context to narrow to col-span-4 when the heatmap shares its row",
  );
});
